    ...
    try:
        ...
        for item in response.get("items", []):
            try:
                ...
                comment_text = ' '.join(comment_text.split())  # Remove extra whitespace
                if comment_text.strip():  # Only add non-empty comments
...
def generate_pie_chart(sentiment_count, video_id):
    try:
        ...
        colors = ["#00ff88", "#ffaa00", "#ff4444"]  # Green, Orange, Red
...
def generate_bar_chart(sentiment_count, video_id):
    try:
        ...
        bars = plt.bar(labels, values, color=colors, edgecolor='white', linewidth=2)
        plt.xlabel("Sentiment", fontsize=12, fontweight='bold', color='white')
        plt.ylabel("Count", fontsize=12, fontweight='bold', color='white')
        plt.title("Sentiment Analysis Results", fontsize=14, fontweight='bold', color='white')
        
        # Add value labels on bars
        for bar, value in zip(bars, values):
            plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.1, 
                    str(value), ha='center', va='bottom', fontweight='bold', color='white')
...
def analyze():
    ...
    # Use default charts if generation failed
